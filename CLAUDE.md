...
**AI:** Google Generative AI (`@google/generative-ai`) — quiz モジュールで PDF を問題形式にパース（`pdfjs-dist` も使用）。
...
- `Quiz.tsx`、`QuizPlay.tsx`、`Timetable.tsx` の `catch` で `setDbError(true)` を呼ぶ
...
- `failedUrls` Set で失敗した URL の再試行を抑止
...
- Firestore への書き込みは 800ms デバウンス（`saveTimeoutRef` + `setTimeout`）
