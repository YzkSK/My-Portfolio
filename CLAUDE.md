...
- **新機能を追加した場合・既存機能を変更した場合も必ずテストコードを書くこと**。Hooks・非同期処理・複雑なロジックは結合テストで動作を担保すること。テストを追加・変更した場合は `docs/README.md` のテスト件数も更新すること
- **機能を追加・変更した場合は `docs/` の該当ドキュメントも必ず更新すること**（`docs/README.md` が索引）。新規ページや共有モジュールを追加した場合は対応する docs ファイルも新規作成し、`docs/README.md` のページ一覧に追記すること
