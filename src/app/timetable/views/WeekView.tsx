            ...
            {weekDays.map((date, di) => {
              ...
              return (
                <div key={`${key}-${periodIndex}-${di}`}
                  ...
                  style={{ height: 60, borderRadius: 8, cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4px 3px', textAlign: 'center', overflow: 'hidden', background: ev ? `var(--tt-event-${ev.colorIdx ?? 0}-bg)` : isTod ? 'var(--tt-today-bg)' : 'var(--tt-bg-card)', border: ev ? 'none' : `1.5px dashed ${isTod ? 'var(--tt-text-muted)' : 'var(--tt-border)'}`, transition: 'opacity 0.1s' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.opacity = '0.8'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.opacity = '1'; }}
                >
                  {ev ? (
                    <>
                      <div style={{ fontSize: 10, fontWeight: 800, color: c!.text, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }}>{ev.name}</div>
                      {ev.room && <div style={{ fontSize: 8, color: c!.text, opacity: 0.75, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }}>{ev.room}</div>}
                    </>
                  ) : <div style={{ fontSize: 12, color: isTod ? 'var(--tt-text-muted)' : 'var(--tt-border)' }}>+</div>}
                </div>
              );
            })}
          </Fragment>
