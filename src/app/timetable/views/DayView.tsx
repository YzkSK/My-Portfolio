      ...
      {periods.map((period, periodIndex) => {
        ...
        return (
          <div key={periodIndex}
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', marginBottom: 6, borderRadius: 8, cursor: 'pointer', minHeight: 60, background: ev ? `var(--tt-event-${ev.colorIdx ?? 0}-bg)` : isTod ? 'var(--tt-today-bg)' : 'var(--tt-bg-card)', border: ev ? 'none' : `1.5px dashed ${isTod ? 'var(--tt-text-muted)' : 'var(--tt-border)'}`, transition: 'opacity 0.1s' }}
