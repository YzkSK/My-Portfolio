...
type Props = {
  ...
  accessToken: string;
  onSave: (folders: DriveFolder[]) => void;
  onClose: () => void;
      ...
      <DialogContent className="max-w-[480px]" aria-describedby={undefined}>
        ...
        <div className="vc-folder-list">
          ...
          {!loading && folders.map(folder => (
            <div key={folder.id} className="vc-folder-item">
              <input
                type="checkbox"
                id={`folder-${folder.id}`}
              ...
              <label htmlFor={`folder-${folder.id}`} style={{ flex: 1, cursor: 'pointer', fontSize: 13 }}>
                {folder.name}
              </label>
              <button
        ...
        <div className="flex gap-2 mt-2">
          ...
          <Button variant="default" className="flex-[2]" onClick={handleSave}>保存</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
